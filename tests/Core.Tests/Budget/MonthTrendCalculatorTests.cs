...
{
    private static Month M(int year, int month, DateOnly start, decimal usdIncome) => new()
    {
        TenantId = Guid.Empty, Year = year, MonthNumber = month, WeekCount = 4, Week1StartDate = start,
        PrimaryIncomeAmount = usdIncome, PrimaryIncomeCurrency = "USD", SecondaryIncomeAmount = 0m, SecondaryIncomeCurrency = "USD",
    };
    ...
    {
        var july = M(2026, 7, new DateOnly(2026, 6, 25), 1000m);
        var june = M(2026, 6, new DateOnly(2026, 5, 28), 1000m);
        ...
        var trend = MonthTrendCalculator.Calculate([july, june], rows, 500m);
    ...
    public void NoRate_SpendStillComputed_IncomeNull()
    {
        ...
        var entry = Assert.Single(MonthTrendCalculator.Calculate([june], [Tx(june.Id, "budgeted", 10_000m)], null));
    ...
    {
        var june = M(2026, 6, new DateOnly(2026, 5, 28), 0m);
