...
public class ReportSliceTests(PostgresFixture fixture) : PostgresTestBase(fixture)
{
    ...
    public async Task Analyze_SingleMonth_GroupsWithNamesAndBudget_FiltersToTheWindow()
    {
        ...
        var byMethod = Assert.Single(report.BudgetByMethod!); // the same plan cut by how the lines are paid (the one line is a card line)
        Assert.Equal(("credit_card", 60_000m, 120m), (byMethod.Key, byMethod.TotalCrc, byMethod.TotalUsd));
