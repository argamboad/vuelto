...
| `docs/QA_TEST_PLAN.md` | Manual QA plan — step-by-step tests across web + all four native platforms (174 cases: smoke + regression + §14a v3-audit adversarial/tenant-isolation + §13c native release checklist) |
...
| `docs/stories/email-ingestion.md` | epic `EMAIL` — port slices P9/P10 (ADR-V002/V010): EMAIL-1 ✅ voucher parser library (`Vuelto.Core.Vouchers` contracts + `SpanishDateParser`/`VoucherText`/`BankVoucherMap` routing-as-data; `Vuelto.Infrastructure.Vouchers` BAC/BN/BN-payment extractors that never throw + validating `VoucherParser`; `AddVoucherParsing()`; real anonymized fixtures); EMAIL-2..6 (connections, readers, staging, suggestions, review queue) to follow — from donor US-025–030 |
