...
| `docs/stories/dashboard.md` | epic `DASH` — app slice P7 (ADR-V004/V006/V007): DASH-1 the month dashboard (pure `DashboardSummaryService` in Core: frozen actuals vs live projections, lines vs actuals, other spending, weekly rows, balance trio, unplanned + pending refunds, envelope reminders, bank × method; `GET /api/months/{id}/summary`, `rate_unavailable` blocks projections; `/dashboard`, `/dashboard/{id}`) — from donor US-005 / US-012 / US-018 / US-055 |
