...
- **CI is proportional to the change (LOCALCI-3, inherited from the platform 2026-09-11).** A job
  ...
  smoke (QA §13c).
- **Forgejo is home (LOCALCI-4, ported from the platform 2026-09-18, ADR-028).** `origin` is the
  private Forgejo (`argamboad/y-el-vuelto`); `github` (`argamboad/vuelto`) is a mirror. Branches, PRs
  and merges happen on Forgejo — never merge on GitHub. `.forgejo/workflows/ci.yml` is a held copy of
  `.github/workflows/ci.yml`: **edit BOTH** (`ForgejoCiParityTests`, R80, fails otherwise; every
  deliberate difference is marked `LOCALCI-4:`). Pushes run gates + `e2e` + native builds; the native
  smokes and the deploys run from *Run workflow* (`smokes` / `deploy` inputs) — a staging deploy
  fast-forwards GitHub's `develop`, then fires the Render hook. Runbook: `docs/DEPLOYMENT.md` §10.
...
- **Two-role RLS posture on a hosted DB (hidden operator tool):** `tools/staging-rls.ps1 -OwnerUrl <neon url>
...
| `docs/QA_TEST_PLAN.md` | Manual QA plan — step-by-step tests across web + all four native platforms (204 cases: smoke + regression + §14a v3-audit adversarial/tenant-isolation + §14c pre-launch gates + §13c native release checklist) |
| `docs/ROADMAP.md` | Sequenced plan — pillars done (JOBS/BILLING/OBS) + the next waves (RBAC, files, GDPR, MFA, …) |
| `docs/UI_REDESIGN_PLAN.md` | **MERGED — PR #63 (2026-09-14): all 11 slices + the additive API fields + the fix set. Follow-ups merged the same day: budget lines carry no bank (PR #64), refund `case_number` column dropped (PR #66), native OAuth trailing-slash fix + Cancel while the browser is out (PR #65)** — the `UI Handout.pdf` presentation-layer redesign read against this codebase: epic `SKIN`, eleven app-side slices shipped as ONE PR with a commit each (11 conforms the fourteen screens the handout never drew), five stale claims in it (incl. three silent removals of shipped behaviour), what it costs in strings/tests/QA, the three design decisions (each with a proposal, each due at its own slice gate), the per-slice QA-plan impact map (§5b), and the show-then-green-light rule before every slice |
| `docs/REPORTS_PDF_INCOME_PLAN.md` | **Plan (owner-confirmed 2026-09-16)** for the report PDF (REPORTS-7, built), "Email me this report" (REPORTS-8, built), income lines with a pay period (INCOME-1, built) and income by member (INCOME-2, built): decisions D1–D9, confirmed assumptions A1–A10, the data-safety rules for the income migration (additive only, parity query, restore point before merge; dropping the old columns is a later owner-gated INCOME-3), and the one-branch / one-commit-per-slice / one-PR working mode |
...
| `docs/stories/budget-settings.md` | epic `BUDGET` — app slice P1 (ADR-V001/V003): BUDGET-1 household budget settings (week start — the payday for weekly pay — and month anchor; the income defaults moved to INCOME-1; `/api/budget-settings`; `WeekBoundaryService` in Core) — from donor US-003 / US-015 |
| `docs/stories/catalog.md` | epic `CATALOG` — app slice P2 (ADR-V008/V009): CATALOG-1 categories + CATALOG-2 banks (soft delete, case-insensitive uniqueness, 409 reactivation offer, seed once in the caller's locale; `/api/categories`, `/api/banks`; `SeedCatalog` in Core) — from donor US-010 / US-013 / US-019 / US-047; CATALOG-3/CARDS-1 the household's cards — brand + last four from the voucher, alias by the household, `VISA-1234` on first sight (`/api/cards`, `/cards`, optional `card_id` on transactions, ADR-V021) |
| `docs/stories/exchange-rate.md` | epic `FX` — app slice P3 (ADR-V006, ADR-V019): FX-1 the day's USD→CRC **buy/sell pair** (BCCR via the Finance Ministry mirror, no key; the one-rate world feed selectable) with the fallback chain (live → stale "as of" → last transaction → 503 `exchange_rate_unavailable`; `GET /api/exchange-rate` with `buy`/`sell`); the direction rule in Core `FxRates` (spend USD → sell, spend CRC → buy, income the mirror; a transaction freezes the side its currency selects); seams `IExchangeRateService` / `IExchangeRateResolver` / `IRecentRateSource`; badge + form prefill — from donor US-014 / US-034 |
...
| `docs/stories/income.md` | epic `INCOME` — owner request (ADR-V023): INCOME-1 household income lines (member, currency, fixed \| variable, pay period `weekly` \| `biweekly` \| `monthly`, amount per payment, biweekly pay days; soft delete, 409 reactivation offer, ordered; `/api/incomes`, `/incomes`), each new month snapshots them into editable `MonthIncome` rows (`IncomeSnapshot` in Core; `PUT /api/months/{id}/income` takes the full list), additive `AddIncomeLines` migration with `tools/backfill-income-lines.sql` + `tools/check-income-parity.sql`, old income columns kept until INCOME-3; INCOME-2 income by member (`IncomeByMember` in Core → `income_by_member` on the category analysis, a table in the PDF; no chart) |
| `docs/stories/ledger.md` | epic `LEDGER` — app slice P5 (ADR-V005/V006/V007): LEDGER-1 months & weeks that exist only through transactions (anchor-window resolve, auto-create with income snapshot, auto-delete) + LEDGER-2 transactions (frozen rate, derived amounts, required bank/category, envelope rules; `/api/months`, `/api/transactions`; `/months`, `/transactions/new`) — P5a; LEDGER-3 expected refunds derived from unplanned essentials, realized as read-only inflows under a conditional flip (`/api/months/{id}/refunds`, `/api/refunds/{id}`) — P5b — from donor US-004/006/007/008/009/012/015, WU-2 |
| `docs/stories/expense-lines.md` | epic `EXPENSES` — app slice P6 (ADR-V007/V008): EXPENSES-1 fixed & variable budget lines (`IExpenseLine`, single-currency budget, required category backing ≤ 1 active line across both lists, optional bank, soft delete + 409 offer, atomic reorder, never seeded; `/api/expenses/{fixed\|variable}`; `/budget`) — from donor US-016 / US-017 / US-054 |
| `docs/stories/dashboard.md` | epic `DASH` — app slice P7 (ADR-V004/V006/V007): DASH-1 the month dashboard (pure `DashboardSummaryService` in Core: frozen actuals vs live projections, lines vs actuals, other spending, weekly rows, the "This month" waterfall (income → class spend → left now → still planned → forecast; ADR-V018 replaced the Expenses/Balance cards, `spent_*` on the summary), unplanned + pending refunds, envelope reminders, bank × method; `GET /api/months/{id}/summary`, `rate_unavailable` blocks projections; `/dashboard`, `/dashboard/{id}`) — from donor US-005 / US-012 / US-018 / US-055; DASH-2 the "Show in" ₡ · $ · both preference — device copy + account copy in the user-keyed `UserDisplaySettings` (`/api/display-settings`, ADR-V020) |
| `docs/stories/reports.md` | epic `REPORTS` — app slice P8 (ADR-V004/V005/V006, platform ADR-010): REPORTS-1 category analysis by class for a month (anchor window, budgets decorated) or a date range (`GET /api/reports/category-analysis`, pure `CategoryAnalysisCalculator`); REPORTS-2 CSV export of a period stored via `IFileStorage` behind a 15-min signed link and delivered through `IFileDownloadLauncher` (`POST /api/reports/transactions/export`, `TransactionCsvWriter`, 4-dp rate); `/reports` + month-page Export button — from donor US-043 / US-044 / WU-4; REPORTS-3 charts (Table/Chart view, ₡/$ switch, inline-SVG `BarChart`/`DonutChart`, "Spend by class" + "Income vs spend" + "Income vs budget" donuts — on the shared `Core` `IncomeCalculator` / `BudgetTotals`, `income` + `budget_total` on the analysis response, null for a range / no rate); REPORTS-4 pace `LineChart` (`spend_by_day`, plan line, today marker), "Month by month" bars (`GET /api/reports/months-trend`, pure `MonthTrendCalculator`), "Spend by bank" / "Card vs account" donuts (`by_bank` / `by_method`, any period); REPORTS-7 the report as a PDF — `POST /api/reports/pdf` rendered by QuestPDF from the page's own figures (pure `ReportPdfModelBuilder`, own `PdfCharts` SVG builders, Nunito embedded) + a landscape appendix of the CSV's rows (`ReportHandler.ExportRowsAsync`), delivered through the CSV's signed link; PDF dialog on `/reports` (ADR-V022); REPORTS-8 "Email me" — `POST /api/reports/pdf/email` queues the same PDF as an attachment to the caller's own address (10/day per person, `ReportEmailRateLimit`); both PDF endpoints follow the language saved in the account; REPORTS-9 the appendix's columns are the reader's choice (`appendix_columns`: category, class, amount, rate, method, bank, source, card, notes — date and payee always; absent = all; the dialog ticks all each time) |
| `docs/stories/email-ingestion.md` | epic `EMAIL` — port slices P9/P10 (ADR-V002/V010/V016): EMAIL-1 ✅ voucher parser library (`Vuelto.Core.Vouchers` + `Vuelto.Infrastructure.Vouchers`, BAC/BN extractors, routing-as-data, real fixtures); EMAIL-2 ✅ user-keyed `EmailConnection` + consent round-trip on the platform's OAuth apps with Data-Protection tokens/state (`/api/email/connections`, anonymous callback, `/email` page, `IUserDataContributor`); EMAIL-3 ✅ Graph + Gmail readers (`IEmailReader`, filters pushed into the provider query, refresh-once, paging/saturation, folder pickers); EMAIL-4 ✅ staging & dedup (`PendingVoucher` + `IngestedVoucher` tombstone with RLS, `VoucherFingerprint`, `VoucherStagingService` with the tenant hop + cursor rules, `EmailPollJob` on the platform scheduler, Sync now); EMAIL-5 ✅ merchant → category rules (`MerchantCategoryMapping` with RLS + unique lower-cased `PatternKey`, pure `MerchantMatcher` longest-wins shared with staging, `/api/merchant-mappings`, `/merchant-mappings` page, learn-on-confirm never overwrites); EMAIL-6 ✅ review queue (`/api/pending-vouchers` list/count/confirm/discard; confirm books through the Core `ITransactionService` contract implemented by the Ledger handler — R7 — inside one unit-of-work scope with a conditional flip, `source = email`; discard is a guarded flip; `/review` page with the nav count badge + dashboard banner; only category/class edited, parser blanks opened) — **epic COMPLETE**; from donor US-025–030 |
...
| `docs/stories/async-jobs.md` | epic `JOBS` ✅ COMPLETE — outbox+dispatcher, inbox, scheduler (ADR-007); JOBS-4 (2026-09-16, synced from platform #235) file attachments on `IEmailSender` (≤ 10 MiB total, validated before enqueue — pass the `CancellationToken` by name) |
...
| `tools/README.md` | **Operator tools runbook** (hidden — no page/route): `snapshot-household.sql` carries one household to another server (what comes across, prerequisites, step by step, dry run, troubleshooting); `staging-rls.ps1` switches a hosted database to the two-role RLS posture in one command (what the two roles are, prerequisites, step by step, troubleshooting) |
| `docs/DEPLOYMENT.md` | Deployment runbook (DEPLOY-2/3) — Render + Neon + Brevo free-tier bring-up; `Dockerfile` + `render.yaml` reference; required env incl. the Production Stripe-key guard; §6 CI-gated auto-deploy (GitHub); **§10 Forgejo as the primary forge — remotes, runners, secrets, what-runs-when, the deploy button (LOCALCI-4)** |
| `docs/stories/gates.md` | epic `GATES` ✅ COMPLETE (ported from the platform 2026-09-14) — **pre-launch gates**, both deployment config, both cleared on launch day (ADR-027): GATES-1 `Billing:Enabled` default-off (billing controllers + provider webhook removed from the application model ⇒ 404; client hides the link and refuses `/billing`; everyone resolves Free; Free seats **3→5**; billing-off seat-limit/full-household copy) · GATES-2 signup green list (`Signup:AllowedEmails`/`AllowedDomains`, empty = open) — the list decides **who may found a household**; a valid pending invitation admits its addressee iff that invitation's tenant **owner** is green-listed. Gates **account creation only, never sign-in**, at `UserService.CreateUserWithTenantAsync`; deliberately does NOT fire at magic-link/OTP **issue** time (user-enumeration oracle) |
