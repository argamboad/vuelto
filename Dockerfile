...
# ---- build ---------------------------------------------------------------------------------------
# Pin the SDK to the single source of truth — global.json (10.0.401) — which generated the committed
...
COPY src/Core/Vuelto.Core.csproj src/Core/packages.lock.json src/Core/
COPY src/Infrastructure/Vuelto.Infrastructure.csproj src/Infrastructure/packages.lock.json src/Infrastructure/
COPY src/Api/Vuelto.Api.csproj src/Api/packages.lock.json src/Api/
COPY src/Shared.Ui/Vuelto.Shared.Ui.csproj src/Shared.Ui/packages.lock.json src/Shared.Ui/
COPY src/Web/Vuelto.Web.csproj src/Web/packages.lock.json src/Web/
...
RUN dotnet restore src/Api/Vuelto.Api.csproj --locked-mode \
 && dotnet restore src/Web/Vuelto.Web.csproj
...
RUN dotnet publish src/Web/Vuelto.Web.csproj -c Release --no-restore -o /publish/web \
 && dotnet publish src/Api/Vuelto.Api.csproj -c Release --no-restore -o /publish/api \
...
ENTRYPOINT ["sh", "-c", "exec dotnet Vuelto.Api.dll --urls http://0.0.0.0:${PORT:-8080}"]
